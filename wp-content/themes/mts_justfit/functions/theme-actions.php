...
?>
    <?php if ($mts_options['mts_footer_logo'] != '') { ?>
        <div class="footer-logo">
            <img src="<?php echo $mts_options['mts_footer_logo']; ?>" alt="<?php bloginfo( 'name' ); ?>" >
        </div>
    <?php } ?>
    <div>&copy; <?php echo date("Y") ?> <?php bloginfo('name'); ?></div>
    <div class="row" id="copyright-note">
        <div class=""><?php echo $mts_options['mts_copyrights']; ?></div>
    </div> 
    <!--end copyrights-->
<?php }
}
...
if (!function_exists('mts_the_breadcrumb')) {
    function mts_the_breadcrumb() {
        ...
        echo home_url();
        echo '" rel="nofollow"><i class="fa fa-home"></i><span style="display:none">'.__('Home', 'mythemeshop' ).'</span>';
        echo '</a></span>';
    ...
    function mts_related_posts() {
        ...
        //if(!empty($mts_options['mts_related_posts'])) { ?>	
    		...
    		$my_query = new wp_query( $args ); if( $my_query->have_posts() ) {
                ...
                <article class="latestPost excerpt  <?php echo (++$j % $posts_per_row == 0) ? 'lasts' : ''; ?>">
            ...
            <header>
              ...
              <div class="post-info">
              <?php if ($mts_options['mts_date_blog'] == '1') { ?><span class="thetime updated"><span itemprop="datePublished"><?php the_time( get_option( 'date_format' ) ); ?></span></span><?php } ?>
                </div>
            </header>
...
if ( ! function_exists('mts_the_postinfo_item' ) ) {
    function mts_the_postinfo_item( $item ) {
        switch ( $item ) {
            case 'author':
            ?>
                <div class="theauthor"><span itemprop="author"><?php the_author_posts_link(); ?></span></div>
            <?php
            break;
            case 'date':
            ?>
        ...
        switch ( $button ) {
            ...
            ?>
                ...
                <span class="share-item twitterbtn">
                    <a href="https://twitter.com/share" class="twitter-share-button" data-via="<?php echo $mts_options['mts_twitter_username']; ?>"><?php _e( 'Tweet', 'mythemeshop' ); ?></a>
                </span>
            <?php
            break;
            case 'gplus':
            ...
            ?>
                ...
                <span class="share-item pinbtn">
                    <a href="http://pinterest.com/pin/create/button/?url=<?php the_permalink(); ?>&media=<?php $thumb = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'large' ); echo $thumb['0']; ?>&description=<?php the_title(); ?>" class="pin-it-button" count-layout="horizontal"><?php _e( 'Pin It', 'mythemeshop' ); ?></a>
                </span>
            <?php
            break;
            case 'linkedin':
