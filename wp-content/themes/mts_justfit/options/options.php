...
class NHP_Options{
	...
	*/
	function _enqueue(){
        ...
        wp_localize_script( 'nhp-opts-js', 'nhpopts', array(
            ...
            'reset_confirm' => __('Are you sure you want to reset ALL options to default?', 'mythemeshop' ),
            ...
            'import_done' => __('Importing proccess finished!', 'mythemeshop' ),
            'import_fail' => __('Importing proccess failed! Please try again.', 'mythemeshop' ),
            'remove_done' => __('Removal proccess finished!', 'mythemeshop' ),
            'remove_fail' => __('Removal proccess failed! Please try again.', 'mythemeshop' ),
            'reloading_page' => __('Reloading page...', 'mythemeshop' ),
            'import_opt_confirm' => __('Are you sure you want to import demo options? All current options will be lost.', 'mythemeshop' ),
            'import_widget_confirm' => __('Are you sure you want to import demo options and widgets? All current options will be lost and existing widgets deactivated.', 'mythemeshop' ),
            'import_all_confirm' => __('Are you sure you want to import demo?', 'mythemeshop' ),
            'remove_all_confirm' => __("Are you sure you want to remove demo options, content, menus and widgets? Please note that all modifications you've made to imported content will be lost", 'mythemeshop' ),
        ) );
	...
	*/
	function _download_options(){
		if(!isset($_GET['secret']) || $_GET['secret'] != md5(AUTH_KEY.SECURE_AUTH_KEY)){wp_die(__( 'Invalid Secret for options use', 'mythemeshop' ));exit;}
		if(!isset($_GET['option'])){wp_die(__( 'No Option Defined', 'mythemeshop' ));exit;}
		...
		echo '<div class="wrap"><h2></h2>';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">';
				...
				/*if ( ! MTS_THEME_WHITE_LABEL ) {*/
					...
					echo '<div class="theme-options-search"><a href="#" class="theme-options-clear-search"><i class="dashicons dashicons-no"></i></a> <input type="text" id="search-theme-options" value="" placeholder="'.esc_attr__('Search Options', 'mythemeshop').'"><a href="#" class="options-search-link"><i class="fa fa-search"></i><span> '.__('Search Options', 'mythemeshop').'</span></a></div>';
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('nhp-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="nhp-opts-imported"><strong>'.__('Settings Imported!', 'mythemeshop' ).'</strong></div>';
						}else{
							echo '<div id="nhp-opts-save"><strong>'.__('Settings Saved!', 'mythemeshop' ).'</strong></div>';
						}
						delete_transient('nhp-opts-saved');
					}
					echo '<div id="nhp-opts-save-warn"><strong>'.__('Settings have changed, you should save them!', 'mythemeshop' ).'</strong></div>';
					echo '<div id="nhp-opts-field-errors"><strong><span></span> '.__('error(s) were found!', 'mythemeshop' ).'</strong></div>';
					
					echo '<div id="nhp-opts-field-warnings"><strong><span></span> '.__('warning(s) were found!', 'mythemeshop' ).'</strong></div>';
				...
				echo '<div id="nhp-opts-sidebar">';
					echo '<ul id="nhp-opts-group-menu">';
						foreach($this->sections as $k => $section){
                            ...
                            if ($k == 4) {
                                echo '<li id="accordion_section_group_li" class="nhp-opts-group-tab-link-li"><a href="javascript:void(0);" id="accordion_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="3" title="Homepage"><i class="fa fa-bars"></i> <span class="section_title">'.__('Homepage','mythemeshop').'</span></a></li>';
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h2>'.__('Import / Export Options', 'mythemeshop' ).'</h2>';
                            ...
                            if (!empty($this->args['presets'])) {
                                ...
                                foreach ( $this->args['presets'] as $key => $preset ) {
                                    echo '<div class="preset" data-nonce="'.$nonce.'" data-demo-id="'.esc_attr($key).'">';
                                        $imported_flag    = '<span class="demo_part_imported">'.__( '(imported)', 'mythemeshop' ).'</span>';
                                        ...
                                        if ( ! empty( $preset['demo'] ) ) echo '<a href="'.$preset['demo'].'" target="_blank" class="demo-link">'.__('View demo', 'mythemeshop' ).'</a>';
                                        ...
                                        echo '<div class="preset-actions-wrap">';
                                        	echo '<button class="button button-secondary import-demo-options-button"'.$disabled_button.'>'.__('Import Theme Options', 'mythemeshop' ).'</button>';
                                        	echo '<button class="button button-secondary import-demo-widgets-button"'.$disabled_button.'>'.__('Import Theme Options & Widgets', 'mythemeshop' ).'</button>';
                                        	if ( !empty( $all_imported ) ) {
                                        		echo '<button class="button button-primary remove-demo-button" data-nonce="'.$nonce.'">'.__('Remove imported data', 'mythemeshop' ).'</button>';
                                        	} else {
                                        		echo '<button class="button button-secondary import-demo-button"'.$disabled_button.'>'.__('Import Theme Options, Widgets & Content', 'mythemeshop' ).'</button>';
                                        	}
                                        echo '</div>';
                                ...
                                echo '<div id="importing-overlay" class="mfp-hide">';
                                	echo '<div id="importing-modal">';
	                                	echo '<div id="importing-modal-header">';
	                                		echo '<h2><span class="spinner is-active"></span>' . __( 'Processing, please wait...', 'mythemeshop' ) . '</h2>';
	                                	echo '</div>';
	                                	echo '<div id="importing-modal-content">';
	                                	echo '</div>';
	                                	echo '<div id="importing-modal-footer">';
	                                		echo '<span id="importing-modal-footer-info">'.__( 'Processing, please wait...', 'mythemeshop' ).'</span>';
	                                		echo '<button id="importing-modal-footer-button" class="button button-primary">'.__( 'Ok', 'mythemeshop' ).'</button>';
                                		echo '</div>';
                                	echo '</div>';
                                echo '</div>';
							...
							echo '<p><a href="#" id="nhp-opts-import-code-button" class="button-secondary">' . esc_html__( 'Import Code', 'mythemeshop' ) . '</a></p>';
					...
					if(true === $this->args['dev_mode']){
						echo '<div id="dev_mode_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h2>'.__('Dev Mode Info', 'mythemeshop' ).'</h2>';
							echo '<div class="nhp-opts-section-desc">';
...
EOF;
	...
	function mts_filesystem_notice() {
		echo '<div class="updated"><p>' . esc_html__( 'You do not have permission to do this.', 'mythemeshop' ) . '</p></div>';
	}
