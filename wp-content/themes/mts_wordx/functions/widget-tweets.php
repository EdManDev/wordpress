...
class mts_widget_recent_tweets extends WP_Widget {
	...
	public function widget($args, $instance) {
		...
		if ( empty( $tweets ) ) { _e( 'Couldn\'t retrieve tweets! Wrong username?', 'mythemeshop' ); echo $after_widget; return; }
	...
	public function relative_time($a) {			
		...
		$week = $day * 7;				
		if(is_numeric($d) && $d > 0) {				
			if($d < 3) return __( "right now", 'mythemeshop' ); //if less then 3 seconds
			if($d < $minute) return sprintf( __("%s seconds ago", 'mythemeshop' ), floor($d) ); //if less then minute
			if($d < $minute * 2) return __( "about 1 minute ago", 'mythemeshop' ); //if less then 2 minutes
			if($d < $hour) return sprintf( __( "%s minutes ago", 'mythemeshop' ), floor( $d / $minute ) ); //if less then hour
			if($d < $hour * 2) return __( "about 1 hour ago", 'mythemeshop' ); //if less then 2 hours
			if($d < $day) return sprintf( __("%s hours ago", 'mythemeshop' ), floor( $d / $hour ) ); //if less then day
			if($d > $day && $d < $day * 2) return __( "yesterday", 'mythemeshop' ); //if more then day, but less then 2 days
			if($d < $day * 365) return sprintf(  __( "%s days ago", 'mythemeshop' ), floor( $d / $day ) ); //if less then year
			return __( "over a year ago", 'mythemeshop' ); //else return more than a year
		}
	}
}	
// register	widget
