...
Author: Eric Alliwp_register_style
...
class mtsGoogleTypography {
	...
	function help_tab() {
		...
		$adding_title					= __('Adding A Collection', MTS_THEME_TEXTDOMAIN );
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', MTS_THEME_TEXTDOMAIN ).'</p>';
		$adding_content			 .= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', MTS_THEME_TEXTDOMAIN ).'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', MTS_THEME_TEXTDOMAIN ).'</a></p>';
		$customizing_title 		= __('Customizing', MTS_THEME_TEXTDOMAIN );
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', MTS_THEME_TEXTDOMAIN ).'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', MTS_THEME_TEXTDOMAIN ).'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', MTS_THEME_TEXTDOMAIN ).'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', MTS_THEME_TEXTDOMAIN ).'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', MTS_THEME_TEXTDOMAIN ).'</li><li><b>'.__('Font Variant', MTS_THEME_TEXTDOMAIN ).'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', MTS_THEME_TEXTDOMAIN ).'</li><li><b>'.__('Font Size', MTS_THEME_TEXTDOMAIN ).'</b> - '.__('The size you would like this font to be.', MTS_THEME_TEXTDOMAIN ).'</li><li><b>'.__('Font Color', MTS_THEME_TEXTDOMAIN ).'</b> - '.__('The color you\'d like to use for this font.', MTS_THEME_TEXTDOMAIN ).'</li><li><b>'.__('CSS Selectors', MTS_THEME_TEXTDOMAIN ).'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', MTS_THEME_TEXTDOMAIN ).'</li></ul>';
		$selectors_title 			= __('CSS Selectors', MTS_THEME_TEXTDOMAIN );
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', MTS_THEME_TEXTDOMAIN ) . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', MTS_THEME_TEXTDOMAIN ) . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', MTS_THEME_TEXTDOMAIN ).':</b> '.__('#selector', MTS_THEME_TEXTDOMAIN ).'</li><li><b>'.__('Classes:', MTS_THEME_TEXTDOMAIN ).':</b> '.__('.selector', MTS_THEME_TEXTDOMAIN ).'</li><li><b>'.__('HTML Tags', MTS_THEME_TEXTDOMAIN ).':</b> '.__('span', MTS_THEME_TEXTDOMAIN ).'</ul>';      
		$selectors_content	 .= '<p><b>'.__('Example', MTS_THEME_TEXTDOMAIN ).':</b> '.__('#selector span.date', MTS_THEME_TEXTDOMAIN ).'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', MTS_THEME_TEXTDOMAIN ) );
		}
		return $links;
	}
	...
	function options_ui() {
		...
		$title								= __('Theme Typography', MTS_THEME_TEXTDOMAIN );
		$loading							= __('Loading Your Collections', MTS_THEME_TEXTDOMAIN );
		$add_new        			= __('Add New Collection', MTS_THEME_TEXTDOMAIN );
		$reset        				= __('Reset Collections', MTS_THEME_TEXTDOMAIN );
		$preview_text   			= __('Type in some text to preview...', MTS_THEME_TEXTDOMAIN );
		$collection_title 			= __('New Collection', MTS_THEME_TEXTDOMAIN );
        //$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', MTS_THEME_TEXTDOMAIN );
		//$font_family_title  	= __('Font family...', MTS_THEME_TEXTDOMAIN );
        $std_font_family_title  	= __('Standard Fonts', MTS_THEME_TEXTDOMAIN );
        $google_font_family_title  	= __('Google Fonts', MTS_THEME_TEXTDOMAIN );
		$font_variant_title 	= __('Variant...', MTS_THEME_TEXTDOMAIN );
		$font_size_title 			= __('Size...', MTS_THEME_TEXTDOMAIN );
        $backup_font_title          = __('Fallback:', MTS_THEME_TEXTDOMAIN );
		$css_selectors_title	= __('CSS Selectors:', MTS_THEME_TEXTDOMAIN );
        $additional_css_title	= __('Additional CSS:', MTS_THEME_TEXTDOMAIN );
		$delete_button_text   = __('Delete', MTS_THEME_TEXTDOMAIN );
		$save_button_text			= __('Save', MTS_THEME_TEXTDOMAIN );
		
		$welcome_title 				= __('Theme Typography', MTS_THEME_TEXTDOMAIN );
		$welcome_subtitle			= __('Get started in 3 steps.', MTS_THEME_TEXTDOMAIN );//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', MTS_THEME_TEXTDOMAIN ).'</a>';
		$step_1_title 				= __('1. Pick A Font', MTS_THEME_TEXTDOMAIN );
		$step_1_desc					= __('Choose from any of the 600+ Google Fonts.', MTS_THEME_TEXTDOMAIN );
		$step_2_title 				= __('2. Customize It', MTS_THEME_TEXTDOMAIN );
		$step_2_desc					= __('Pick a size, variant, color and more.', MTS_THEME_TEXTDOMAIN );
		$step_3_title 				= __('3. Attach It', MTS_THEME_TEXTDOMAIN );
		$step_3_desc					= __('Attach your font to any CSS selector(s).', MTS_THEME_TEXTDOMAIN );
		$year									= date("Y");
        $themename = MTS_THEME_NAME;
        
        $character_sets_title = __('Character sets', MTS_THEME_TEXTDOMAIN );
        $character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', MTS_THEME_TEXTDOMAIN );
        ...
        $character_sets = array(
            'latin' => __('Latin', MTS_THEME_TEXTDOMAIN ),
            'latin-ext' => __('Latin Extended', MTS_THEME_TEXTDOMAIN ),
            'cyrillic' => __('Cyrillic', MTS_THEME_TEXTDOMAIN ),
            'cyrillic-ext' => __('Cyrillic Extended', MTS_THEME_TEXTDOMAIN ),
            'greek' => __('Greek', MTS_THEME_TEXTDOMAIN ),
            'greek-ext' => __('Greek Extended', MTS_THEME_TEXTDOMAIN ),
            'vietnamese' => __('Vietnamese', MTS_THEME_TEXTDOMAIN ),
            'khmer' => __('Khmer', MTS_THEME_TEXTDOMAIN ),
            'devanagari' => __('Devanagari', MTS_THEME_TEXTDOMAIN ),
        );
...
EOT;
        ...
        $typography_settings .= '</fieldset></td></tr></tbody></table>';
	...
	public function save_image( $image, $font ) {
		...
		$dir = dirname( $this->dir_path.'/images/fonts/png' );
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
	            'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", MTS_THEME_TEXTDOMAIN ),
	            'delete_confirm' => __("Are you sure you want to delete this collection?", MTS_THEME_TEXTDOMAIN )
	        ));
